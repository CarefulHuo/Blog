...
public class CartHandler {
	...
	public String createOrder(Order order, HttpSession session,Model model){
		...
		for (Product p : cart.keySet()) {
			...
			cartService.addOrderItem(item);
		}
		model.addAttribute("order", order);
		cartService.createOrder(order);
		session.removeAttribute("cart");
		return "/client/confirm.jsp";
	}
	
	/*@RequestMapping("/pay1.do")
	public void pay1(String id,String money, String name,HttpServletResponse response) throws  Exception {
		//获得初始化的AlipayClient
	...
	*/
	...
	public void pay(String id,String money,String name,HttpServletResponse response,HttpServletRequest request) throws Exception{
		//获得初始化的AlipayClient
		AlipayClient alipayClient = new DefaultAlipayClient(AlipayConfig.gatewayUrl, AlipayConfig.app_id, AlipayConfig.merchant_private_key, "json", AlipayConfig.charset, AlipayConfig.alipay_public_key, AlipayConfig.sign_type);
