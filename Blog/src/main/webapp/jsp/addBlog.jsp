...
<body>
	...
	<div class="row">
		...
		<div class="col-sm-10">
			...
			<form id="blogForm" method="post" action="${pageContext.request.contextPath}/blogAdmin/addBlog.action?addId=0" role="form">
				<hr>
				<div class="form-group">
				<select  class="from-control selectpicker select-danger" name="b_category">
				  <option  value="0">请选择类别</option>
				  <c:forEach items="${categorys }" var="category">
					<option id="category" value="${category.c_id }">${category.c_name }</option>
				  </c:forEach>
			    </select>
			    &nbsp;&nbsp;&nbsp;&nbsp;
				<select  class="from-control selectpicker select-danger" name="b_label">
				  ...
				  <c:forEach items="${labels }" var="label">
					<option id="label" value="${label.labelId }">${label.labelName }</option>
				  </c:forEach>
			    </select>
				</div>
				<hr>
				<div class="form-group">
					...
					<input id="title" class="col-lg col-lg-9" aria-describaday="basic-addon" type="username" name="b_title">
				</div>
				<hr>
				<hr>
				<textarea  name="b_content" id="blog"></textarea>
